...
def close_trade(trade_id: int, exit_price: float, exit_reason: str, timestamp: datetime = None, db_path: str = None):
    ...
    if trade:
        ...
        conn.execute("""
            ...
            WHERE id = ?
        """, (exit_price, pnl, "win" if pnl > 0 else "loss", exit_reason, now.strftime("%H:%M:%S"), trade_id))
        conn.commit()
    conn.close()
...
def get_today_pnl(mode: str = None, date_override: str = None, db_path: str = None) -> Dict:
    ...
    closed = wins + losses
    return {
        "total_pnl": round(total_pnl, 2), "total_trades": len(trades),
...
def get_all_time_pnl(mode: str = None, date_from: str = None, date_to: str = None, db_path: str = None) -> Dict:
    conn = get_connection(db_path)
    query = "SELECT SUM(pnl) as total_pnl, COUNT(*) as total_trades, SUM(CASE WHEN status = 'win' THEN 1 ELSE 0 END) as wins, SUM(CASE WHEN status = 'loss' THEN 1 ELSE 0 END) as losses FROM trades WHERE status != 'open'"
    params = []
    if mode: query += " AND mode = ?"; params.append(mode)
    if date_from: query += " AND date >= ?"; params.append(date_from)
    if date_to: query += " AND date <= ?"; params.append(date_to)
    ...
    conn.close()
    pnl, trades, wins, losses = row["total_pnl"] or 0, row["total_trades"] or 0, row["wins"] or 0, row["losses"] or 0
    return {"all_time_pnl": round(pnl, 2), "all_time_trades": trades, "all_time_win_rate": round((wins/trades*100) if trades > 0 else 0, 1), "wins": wins, "losses": losses}

def get_yearly_summary(mode: str = None, date_from: str = None, date_to: str = None, db_path: str = None) -> List[Dict]:
    conn = get_connection(db_path)
    query = "SELECT STRFTIME('%Y', date) as year, SUM(pnl) as pnl, COUNT(*) as trades, SUM(CASE WHEN status = 'win' THEN 1 ELSE 0 END) as wins, SUM(CASE WHEN status = 'loss' THEN 1 ELSE 0 END) as losses, SUM(CASE WHEN pnl > 0 THEN pnl ELSE 0 END) as gross_profit, SUM(CASE WHEN pnl < 0 THEN ABS(pnl) ELSE 0 END) as gross_loss FROM trades WHERE status != 'open'"
    params = []
    if mode: query += " AND mode = ?"; params.append(mode)
    if date_from: query += " AND date >= ?"; params.append(date_from)
    if date_to: query += " AND date <= ?"; params.append(date_to)
